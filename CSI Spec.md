...
| GetPluginCapabilities | Both | Reports which optional CSI features this plugin supports. | N/A | Pending testing |
...
| ControllerGetCapabilities | Controller | Reports which controller RPCs this plugin implements. | N/A | Pending testing — over-advertises |
...
| NodeGetCapabilities | Node | Reports which node RPCs this plugin implements. | N/A | Pending testing |
| NodeGetInfo | Node | Reports node identity/topology info used for scheduling and attach decisions. | N/A | Pending testing |

**ControllerGetCapabilities over-advertises.** It currently claims PUBLISH_UNPUBLISH_VOLUME,
EXPAND_VOLUME, CREATE_DELETE_SNAPSHOT, and LIST_SNAPSHOTS, none of which are implemented — so the
sidecars will call RPCs that return Unimplemented. Trim the advertised list to match reality, or
land the missing RPCs, before running this in a cluster.
