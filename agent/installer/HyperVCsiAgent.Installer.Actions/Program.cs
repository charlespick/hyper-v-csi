...
// The whole exe only ever runs on the Windows host being installed onto -
// self-contained win-x64 publish, never anything else - so it is
// Windows-only end to end rather than needing per-call platform guards
// around GrantCertificateAccessCommand's certificate-store APIs.
...
// a plain exe - immediate, from the execute sequence, for wizard-entered
// validation, and deferred/elevated, later in that same execute sequence, for
// the writes (including generate-server-cert, which needs that same
// elevation to import into LocalMachine\My) - sidesteps that entirely and
// lets every subcommand below share the exact validation and options types
// the agent itself binds and enforces at startup.
if (args.Length == 0)
{
    Console.Error.WriteLine("usage: HyperVCsiAgent.Installer.Actions <validate-cert|validate-thumbprints|grant-cert-access|grant-logon-as-service|open-firewall-port|close-firewall-port|write-config|generate-server-cert> [options]");
    return 1;
}
