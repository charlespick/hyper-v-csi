...
Quick reference: every CSI RPC this driver implements, where it's
implemented, and what its idempotency key is. The operation type is never
baked into the key — it travels as a separate field, and the agent dedupes
in-flight jobs on the (operation type, idempotency key) pair.
...
- [Controller RPC design notes](controller-rpc-notes.md)
- [Node RPC design notes](node-rpc-notes.md)
- [Node identity resolution and attach](node-identity-and-attach.md)
- [Host CIM calls and timeouts](host-cim-and-timeouts.md)
